...
class EmployeeRepository:
    ...
    def _find_by_id(self, conn, employee_id):
        with conn.cursor(row_factory=class_row(Employee)) as cur:
            cur.execute("""
                SELECT
                    id_employee,
    ...
    def find_by_email(self, email, conn=None):
        if conn:
            return self._find_by_email(conn, email)

        with get_connection() as conn:
            return self._find_by_email(conn, email)
    
    def _find_by_email(self, conn, email):
        with conn.cursor(row_factory=class_row(Employee)) as cur:
            cur.execute("""
                SELECT
                    id_employee,
    ...
    def create(self, first_name, last_name, email, phone, birth_date, conn=None):
        if conn:
            return self._create(conn, first_name, last_name, email, phone, birth_date)

        with get_connection() as conn:
            return self._create(conn, first_name, last_name, email, phone, birth_date)
    
    def _create(self, conn, first_name, last_name, email, phone, birth_date):
        with conn.cursor(row_factory=class_row(Employee)) as cur:
            cur.execute("""
    ...
    def delete(self, employee_id, conn=None):
        if conn:
            return self._delete(conn, employee_id)

        with get_connection() as conn:
            return self._delete(conn, employee_id)

    def _delete(self, conn, employee_id):
        with conn.cursor(row_factory=class_row(Employee)) as cur:
            cur.execute("""
